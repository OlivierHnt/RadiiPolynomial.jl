...
As a rule of thumb, it is recommended to compute:
- [`Y`](@ref) and [`Z₁`](@ref) if the uniqueness is not important (e.g. it is achieved analytically);
- [`Y`](@ref), [`Z₁`](@ref) and [`Z₂`](@ref) otherwise.
...
Moreover, we define the *truncation operator*
