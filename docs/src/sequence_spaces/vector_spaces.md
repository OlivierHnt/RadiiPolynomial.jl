...
For a given order ``n``, a [`Chebyshev`](@ref) sequence space is the span of ``\{\phi_0, 2\phi_1, \dots, 2\phi_n\}`` where ``\phi_0(t) := 1``, ``\phi_1(t) := t`` and ``\phi_k(t) := 2 t \phi_{k-1}(t) - \phi_{k-2}(t)`` for ``k = 2, \dots, n`` and ``t \in [-1, 1]``. The normalization by a factor ``2`` is done for convenience.
