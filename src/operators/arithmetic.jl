## arithmetic operations +, - between operators

Base.:+(A::Operator) = Operator(A.domain, A.range, +(A.coefficients))
Base.:-(A::Operator) = Operator(A.domain, A.range, -(A.coefficients))

function Base.:+(A::Operator, B::Operator)
    domain, range = A.domain ∪ B.domain, A.range ∪ B.range
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, range, Matrix{CoefType}(undef, length(range), length(domain)))
    if A.domain == B.domain && A.range == B.range
        @. C.coefficients = A.coefficients + B.coefficients
        return C
    elseif A.domain ⊆ B.domain && A.range == B.range
        @. C.coefficients = B.coefficients
        indices = eachindex(A.domain)
        @inbounds view(C, :, indices) .+= view(A, :, indices)
        return C
    elseif B.domain ⊆ A.domain && A.range == B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .+= view(B, :, indices)
        return C
    elseif A.domain == B.domain && A.range ⊆ B.range
        @. C.coefficients = B.coefficients
        indices = eachindex(A.range)
        @inbounds view(C, indices, :) .+= view(A, indices, :)
        return C
    elseif A.domain == B.domain && B.range ⊆ A.range
        @. C.coefficients = A.coefficients
        indices = eachindex(B.range)
        @inbounds view(C, indices, :) .+= view(B, indices, :)
        return C
    elseif A.domain ⊆ B.domain && A.range ⊆ B.range
        @. C.coefficients = B.coefficients
        indices_domain = eachindex(A.domain)
        indices_range = eachindex(A.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(A, indices_range, indices_domain)
        return C
    elseif B.domain ⊆ A.domain && B.range ⊆ A.range
        @. C.coefficients = A.coefficients
        indices_domain = eachindex(B.domain)
        indices_range = eachindex(B.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(B, indices_range, indices_domain)
        return C
    else
        C.coefficients .= zero(CoefType)
        indices_A_domain = eachindex(A.domain)
        indices_A_range = eachindex(A.range)
        @inbounds for β ∈ indices_A_domain, α ∈ indices_A_range
            C[α,β] = A[α,β]
        end
        indices_B_domain = eachindex(B.domain)
        indices_B_range = eachindex(B.range)
        @inbounds for β ∈ indices_B_domain, α ∈ indices_B_range
            C[α,β] += B[α,β]
        end
        return C
    end
end

function Base.:-(A::Operator, B::Operator)
    domain, range = A.domain ∪ B.domain, A.range ∪ B.range
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, range, Matrix{CoefType}(undef, length(range), length(domain)))
    if A.domain == B.domain && A.range == B.range
        @. C.coefficients = A.coefficients - B.coefficients
        return C
    elseif A.domain ⊆ B.domain && A.range == B.range
        @. C.coefficients = -B.coefficients
        indices = eachindex(A.domain)
        @inbounds view(C, :, indices) .+= view(A, :, indices)
        return C
    elseif B.domain ⊆ A.domain && A.range == B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .-= view(B, :, indices)
        return C
    elseif A.domain == B.domain && A.range ⊆ B.range
        @. C.coefficients = -B.coefficients
        indices = eachindex(A.range)
        @inbounds view(C, indices, :) .+= view(A, indices, :)
        return C
    elseif A.domain == B.domain && B.range ⊆ A.range
        @. C.coefficients = A.coefficients
        indices = eachindex(B.range)
        @inbounds view(C, indices, :) .-= view(B, indices, :)
        return C
    elseif A.domain ⊆ B.domain && A.range ⊆ B.range
        @. C.coefficients = -B.coefficients
        indices_domain = eachindex(A.domain)
        indices_range = eachindex(A.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(A, indices_range, indices_domain)
        return C
    elseif B.domain ⊆ A.domain && B.range ⊆ A.range
        @. C.coefficients = A.coefficients
        indices_domain = eachindex(B.domain)
        indices_range = eachindex(B.range)
        @inbounds view(C, indices_range, indices_domain) .-= view(B, indices_range, indices_domain)
        return C
    else
        C.coefficients .= zero(CoefType)
        indices_A_domain = eachindex(A.domain)
        indices_A_range = eachindex(A.range)
        @inbounds for β ∈ indices_A_domain, α ∈ indices_A_range
            C[α,β] = A[α,β]
        end
        indices_B_domain = eachindex(B.domain)
        indices_B_range = eachindex(B.range)
        @inbounds for β ∈ indices_B_domain, α ∈ indices_B_range
            C[α,β] -= B[α,β]
        end
        return C
    end
end

## composition

Base.:∘(A::Operator, B::Operator) = *(A, B)

function Base.:*(A::Operator, B::Operator)
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(B.domain, A.range, Matrix{CoefType}(undef, length(A.range), length(B.domain)))
    if A.domain == B.range
        mul!(C.coefficients, A.coefficients, B.coefficients)
        return C
    elseif A.domain ⊆ B.range
        @inbounds mul!(C.coefficients, A.coefficients, view(B, eachindex(A.domain), :))
        return C
    elseif B.range ⊆ A.domain
        @inbounds mul!(C.coefficients, view(A, :, eachindex(B.range)), B.coefficients)
        return C
    else
        indices = eachindex(A.domain ∩ B.range)
        @inbounds mul!(C.coefficients, view(A, :, indices), view(B, indices, :))
        return C
    end
end

## inverse

Base.inv(A::Operator) = Operator(A.range, A.domain, inv(A.coefficients))

## arithmetic operations +, -, *, /, \ with field elements

function Base.:+(A::Operator, b)
    CoefType = promote_type(eltype(A), typeof(b))
    C = Operator(A.domain, A.range, Matrix{CoefType}(undef, length(A.range), length(A.domain)))
    @. C.coefficients = A.coefficients
    @inbounds for α ∈ eachindex(A.domain ∩ A.range)
        C[α,α] += b
    end
    return C
end

Base.:+(b, A::Operator) = +(A, b)

function Base.:-(A::Operator, b)
    CoefType = promote_type(eltype(A), typeof(b))
    C = Operator(A.domain, A.range, Matrix{CoefType}(undef, length(A.range), length(A.domain)))
    @. C.coefficients = A.coefficients
    @inbounds for α ∈ eachindex(A.domain ∩ A.range)
        C[α,α] -= b
    end
    return C
end

function Base.:-(b, A::Operator)
    CoefType = promote_type(eltype(A), typeof(b))
    C = Operator(A.domain, A.range, Matrix{CoefType}(undef, length(A.range), length(A.domain)))
    @. C.coefficients = -A.coefficients
    @inbounds for α ∈ eachindex(A.domain ∩ A.range)
        C[α,α] += b
    end
    return C
end

Base.:*(A::Operator, b) = Operator(A.domain, A.range, *(A.coefficients, b))
Base.:*(b, A::Operator) = Operator(A.domain, A.range, *(b, A.coefficients))

Base.:/(A::Operator, b) = Operator(A.domain, A.range, /(A.coefficients, b))
Base.:\(b, A::Operator) = Operator(A.domain, A.range, \(b, A.coefficients))

## arithmetic operations +̄, -̄ between operators

function +̄(A::Operator, B::Operator)
    domain, range = A.domain ∪̄ B.domain, A.range ∪̄ B.range
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, range, Matrix{CoefType}(undef, length(range), length(domain)))
    if A.domain == B.domain && A.range == B.range
        @. C.coefficients = A.coefficients + B.coefficients
        return C
    elseif A.domain ⊆ B.domain && A.range == B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(A.domain)
        @inbounds view(C, :, indices) .+= view(B, :, indices)
        return C
    elseif B.domain ⊆ A.domain && A.range == B.range
        @. C.coefficients = B.coefficients
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .+= view(A, :, indices)
        return C
    elseif A.domain == B.domain && A.range ⊆ B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(A.range)
        @inbounds view(C, indices, :) .+= view(B, indices, :)
        return C
    elseif A.domain == B.domain && B.range ⊆ A.range
        @. C.coefficients = B.coefficients
        indices = eachindex(B.range)
        @inbounds view(C, indices, :) .+= view(A, indices, :)
        return C
    elseif A.domain ⊆ B.domain && A.range ⊆ B.range
        @. C.coefficients = A.coefficients
        indices_domain = eachindex(A.domain)
        indices_range = eachindex(A.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(B, indices_range, indices_domain)
        return C
    elseif B.domain ⊆ A.domain && B.range ⊆ A.range
        @. C.coefficients = B.coefficients
        indices_domain = eachindex(B.domain)
        indices_range = eachindex(B.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(A, indices_range, indices_domain)
        return C
    else
        C.coefficients .= zero(CoefType)
        indices_A_domain = eachindex(A.domain ∪̄ domain)
        indices_A_range = eachindex(A.range ∪̄ range)
        @inbounds for β ∈ indices_A_domain, α ∈ indices_A_range
            C[α,β] = A[α,β]
        end
        indices_B_domain = eachindex(B.domain ∪̄ domain)
        indices_B_range = eachindex(B.range ∪̄ range)
        @inbounds for β ∈ indices_B_domain, α ∈ indices_B_range
            C[α,β] += B[α,β]
        end
        return C
    end
end

function -̄(A::Operator, B::Operator)
    domain, range = A.domain ∪̄ B.domain, A.range ∪̄ B.range
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, range, Matrix{CoefType}(undef, length(range), length(domain)))
    if A.domain == B.domain && A.range == B.range
        @. C.coefficients = A.coefficients - B.coefficients
        return C
    elseif A.domain ⊆ B.domain && A.range == B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(A.domain)
        @inbounds view(C, :, indices) .-= view(B, :, indices)
        return C
    elseif B.domain ⊆ A.domain && A.range == B.range
        @. C.coefficients = -B.coefficients
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .+= view(A, :, indices)
        return C
    elseif A.domain == B.domain && A.range ⊆ B.range
        @. C.coefficients = A.coefficients
        indices = eachindex(A.range)
        @inbounds view(C, indices, :) .-= view(B, indices, :)
        return C
    elseif A.domain == B.domain && B.range ⊆ A.range
        @. C.coefficients = -B.coefficients
        indices = eachindex(B.range)
        @inbounds view(C, indices, :) .+= view(A, indices, :)
        return C
    elseif A.domain ⊆ B.domain && A.range ⊆ B.range
        @. C.coefficients = A.coefficients
        indices_domain = eachindex(A.domain)
        indices_range = eachindex(A.range)
        @inbounds view(C, indices_range, indices_domain) .-= view(B, indices_range, indices_domain)
        return C
    elseif B.domain ⊆ A.domain && B.range ⊆ A.range
        @. C.coefficients = -B.coefficients
        indices_domain = eachindex(B.domain)
        indices_range = eachindex(B.range)
        @inbounds view(C, indices_range, indices_domain) .+= view(A, indices_range, indices_domain)
        return C
    else
        C.coefficients .= zero(CoefType)
        indices_A_domain = eachindex(A.domain ∪̄ domain)
        indices_A_range = eachindex(A.range ∪̄ range)
        @inbounds for β ∈ indices_A_domain, α ∈ indices_A_range
            C[α,β] = A[α,β]
        end
        indices_B_domain = eachindex(B.domain ∪̄ domain)
        indices_B_range = eachindex(B.range ∪̄ range)
        @inbounds for β ∈ indices_B_domain, α ∈ indices_B_range
            C[α,β] -= B[α,β]
        end
        return C
    end
end

## some mixed operations involing functionals and operators

Base.:∘(A::Functional, B::Operator) = *(A, B)

function Base.:*(A::Functional, B::Operator)
    @assert A.domain == B.range # NOTE: should be relaxed
    CoefType = promote_type(eltype(A), eltype(B))
    len = length(B.domain)
    C = Functional(B.domain, Vector{CoefType}(undef, len))
    @inbounds for i ∈ 1:len
        C.coefficients[i] = mapreduce(*, +, A.coefficients, view(B.coefficients, :, i))
    end
    return C
end

Base.:∘(b::Sequence, A::Functional) = *(b, A)

function Base.:*(b::Sequence, A::Functional)
    CoefType = promote_type(eltype(b), eltype(A))
    C = Operator(A.domain, b.space, Matrix{CoefType}(undef, length(b.space), length(A.domain)))
    indices_domain = eachindex(A.domain)
    indices_space = eachindex(b.space)
    @inbounds for β ∈ indices_domain, α ∈ indices_space
        C[α,β] = b[α]*A[β]
    end
    return C
end

Base.:+(B::Operator, A::Functional) = +(A, B)

function Base.:+(A::Functional, B::Operator)
    domain = A.domain ∪ B.domain
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, B.range, Matrix{CoefType}(undef, length(B.range), length(domain)))
    idx₀ = _constant_index(B.range)
    if A.domain == B.domain
        @. C.coefficients = B.coefficients
        @inbounds view(C, idx₀, :) .+= A.coefficients
        return C
    elseif A.domain ⊆ B.domain
        @. C.coefficients = B.coefficients
        @inbounds view(C, idx₀, eachindex(A.domain)) .+= A.coefficients
        return C
    elseif B.domain ⊆ A.domain
        C.coefficients .= zero(CoefType)
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .= view(B, :, indices)
        @inbounds view(C, idx₀, :) .+= A.coefficients
        return C
    else
        C.coefficients .= zero(CoefType)
        @inbounds for α ∈ eachindex(B.domain)
            view(C, :, α) .= view(B, :, α)
        end
        @inbounds for α ∈ eachindex(A.domain)
            C[idx₀,α] += A[α]
        end
        return C
    end
end

function Base.:-(A::Functional, B::Operator)
    domain = A.domain ∪ B.domain
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, B.range, Matrix{CoefType}(undef, length(B.range), length(domain)))
    idx₀ = _constant_index(B.range)
    if A.domain == B.domain
        @. C.coefficients = -B.coefficients
        @inbounds view(C, idx₀, :) .+= A.coefficients
        return C
    elseif A.domain ⊆ B.domain
        @. C.coefficients = -B.coefficients
        @inbounds view(C, idx₀, eachindex(A.domain)) .+= A.coefficients
        return C
    elseif B.domain ⊆ A.domain
        C.coefficients .= zero(CoefType)
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .= .-view(B, :, indices)
        @inbounds view(C, idx₀, :) .+= A.coefficients
        return C
    else
        C.coefficients .= zero(CoefType)
        @inbounds for α ∈ eachindex(B.domain)
            view(C, :, α) .= .-view(B, :, α)
        end
        @inbounds for α ∈ eachindex(A.domain)
            C[idx₀,α] += A[α]
        end
        return C
    end
end

function Base.:-(B::Operator, A::Functional)
    domain = A.domain ∪ B.domain
    CoefType = promote_type(eltype(A), eltype(B))
    C = Operator(domain, B.range, Matrix{CoefType}(undef, length(B.range), length(domain)))
    idx₀ = _constant_index(B.range)
    if A.domain == B.domain
        @. C.coefficients = B.coefficients
        @inbounds view(C, idx₀, :) .-= A.coefficients
        return C
    elseif A.domain ⊆ B.domain
        @. C.coefficients = B.coefficients
        @inbounds view(C, idx₀, eachindex(A.domain)) .-= A.coefficients
        return C
    elseif B.domain ⊆ A.domain
        C.coefficients .= zero(CoefType)
        indices = eachindex(B.domain)
        @inbounds view(C, :, indices) .= view(B, :, indices)
        @inbounds view(C, idx₀, :) .-= A.coefficients
        return C
    else
        C.coefficients .= zero(CoefType)
        @inbounds for α ∈ eachindex(B.domain)
            view(C, :, α) .= view(B, :, α)
        end
        @inbounds for α ∈ eachindex(A.domain)
            C[idx₀,α] -= A[α]
        end
        return C
    end
end
