...
_similar(x::Number) = zero(x)
_similar(x) = similar(x)

_similar_linop(x::Number) = zero(x)
...
function newton!(F_DF!, x, F, DF; tol::Real = 1e-12, maxiter::Int = 15, convergence_criterion::ConvergenceCriterion = ResidualTolCriterion(), verbose::Bool = false)
    ...
    AF = DF \ F
    nAF = norm(AF, Inf)
    conv_crit, val = convergence_criterion(nF, nAF, tol, ϵ)
    verbose && _print_iter!(t, maxiter, 0, nF, nAF, val)
    conv_crit && return x, true
    ...
    while i ≤ maxiter && isfinite(nF)
        ...
        nAF = norm(AF, Inf)
        conv_crit, val = convergence_criterion(nF, nAF, tol, ϵ)
        verbose && _print_iter!(t, maxiter, i, nF, nAF, val)
        conv_crit && return x, true
        i += 1
    end
...
function _print_iter!(t, maxiter, i, nF, nAF, val)
    sqrt_val = sqrt(val)
    color_norm(x) = ifelse(x < sqrt_val, "\e[32m", ifelse(x < sqrt(sqrt_val), "\e[33m", "\e[31m")) # green/yellow/red
    reset_color = "\e[0m"
