...
function _orbit_representatives(sym::Group, space) # slow
    ...
    for (i, k) ∈ enumerate(inds)
        ...
        for g ∈ sym_elements
            k_g = g.lattice_aut(k)
            # the truncation must be a union of orbits, otherwise an orbit is only partially stored
            _checkbounds_indices(k_g, space) || return throw(ArgumentError("the indices of $space are not invariant under the symmetry group: the orbit of $k contains $k_g"))
            if k_g > k_rep
                k_rep = k_g
            end
        end
...
function _compute_rep_pos_cocycle(sym::Group, inds, k_reps, reps)
    ...
    return map(enumerate(inds)) do (i, k)
        ...
        k_rep = k_reps[i]
        q = get(positions, k_rep, 0) # 0: invalid orbit
        for g ∈ sym_elements
...
coefficient per orbit. With no group given, the trivial one is used. The indices
    ...
    function SymmetricSpace(space::S, sym::G) where {S<:NoSymSpace,G<:Group}
        ...
        reps, rep_pos_cocycle = lock(_orbit_cache_lock) do
            get!(_orbit_cache, (inds, sym)) do
                k_reps = _orbit_representatives(sym, space)
...
quarter turn and a diagonal reflection. Both factors must have the same order,
since the quarter turn exchanges the two indices.
