...
The action of a symmetry on the indices, given by an integer matrix. Callable on an index or a tuple of indices.
...
The action of a symmetry on a coefficient: multiplication by `amplitude`, together with a `phase` measured in units of ``\\pi`` and stored modulo `2`.
...
Combine an [`IndexAction`](@ref) together with the [`CoefAction`](@ref) applied to the corresponding coefficient. Elements compose with `∘`.
...
The group generated by the given elements. It is closed under composition at construction, so [`elements`](@ref) returns every element and not only the generators.
...
Return the set of [`GroupElement`](@ref)s of `g`. The group is closed under composition at construction, so this is the full group and not just the generators it was built from.
...
`indices` returns the orbit representatives, so `dimension` shrinks accordingly, and norms account for the multiplicity of each orbit. Some symmetries are available through [`evensym`](@ref), [`oddsym`](@ref) and [`d4sym`](@ref); build a [`Group`](@ref) directly for
anything else.
...
Return the underlying space of `s` without its symmetry. Acts as the identity on a space that carries no symmetry.
...
Return the [`Group`](@ref) that `s` is invariant under. A space carrying no symmetry returns the trivial group.

See also: [`SymmetricSpace`](@ref), [`desymmetrize`](@ref) and [`elements`](@ref).
"""
