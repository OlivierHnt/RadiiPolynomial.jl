@testset "Norm" begin
    ...
    @test norm(a_geo, Weightedℓ¹(GeometricWeights(ν))) ==
        opnorm(project(Multiplication(a_geo), space(a_geo), space(a_geo), Float64), Weightedℓ¹(GeometricWeights(ν))) ≈
        n+1
    @test norm(a_alg, Weightedℓ¹(AlgebraicWeights(p))) ==
        opnorm(project(Multiplication(a_alg), space(a_alg), space(a_alg), Float64), Weightedℓ¹(AlgebraicWeights(p))) ≈
        n+1
    ...
    @test norm(a_geo, Weightedℓ¹( (GeometricWeights(ν), GeometricWeights(ν), GeometricWeights(ν)) )) ==
        opnorm(project(Multiplication(a_geo), space(a_geo), space(a_geo), Float64), Weightedℓ¹( (GeometricWeights(ν), GeometricWeights(ν), GeometricWeights(ν)) ))
    @test norm(a_alg, Weightedℓ¹( (AlgebraicWeights(p), AlgebraicWeights(p), AlgebraicWeights(p)) )) ==
        opnorm(project(Multiplication(a_alg), space(a_alg), space(a_alg), Float64), Weightedℓ¹( (AlgebraicWeights(p), AlgebraicWeights(p), AlgebraicWeights(p)) ))
    ...
    X = NormedCartesianSpace((ℓ∞(), NormedCartesianSpace(Weightedℓ¹((GeometricWeights(1.0), GeometricWeights(1.0), GeometricWeights(1.0))), ℓ∞())), ℓ∞())
